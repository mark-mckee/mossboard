...
const error    = ref('');
const form     = ref({ name: '', service_ids: [] });
...
async function fetchAll() {
  const [tokRes, svcRes] = await Promise.all([fetch('/api/v1/admin/tokens'), fetch('/api/v1/admin/services')]);
...
async function createToken() {
  error.value = '';
  const res = await fetch('/api/v1/admin/tokens', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(form.value) });
  if (res.ok) { newToken.value = await res.json(); showForm.value = false; form.value = { name: '', service_ids: [] }; await fetchAll(); }
  else { error.value = 'Failed to create.'; }
}

async function toggleToken(token) {
  await fetch(`/api/v1/admin/tokens/${token.id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ active: !token.active }) });
  await fetchAll();
}
...
// Build a label "ServiceName (SectionName)" from services_info
function serviceLabel(info) {
  return info.section_name ? `${info.name}  (${info.section_name})` : info.name;
}

function serviceLabels(token) {
  if (token.services_info && token.services_info.length) {
    return token.services_info.map(serviceLabel).join(', ');
  }
  return 'All services';
}
...
onMounted(async () => { await fetchAll(); buildServicesBySection(); });
</script>
...
<template>
  <div class="p-8">
    <div class="flex items-center justify-between mb-6">
      ...
      <button @click="showForm = !showForm; newToken = null" class="bg-gray-800 hover:bg-gray-700 dark:bg-gray-700 dark:hover:bg-gray-600 text-white text-xs px-3 py-1.5 rounded-lg transition-colors">+ New Token</button>
    </div>
      ...
      <div class="space-y-3">
        ...
        <div>
          <label class="block text-xs text-gray-500 dark:text-gray-400 mb-2">
            Authorized services <span class="text-gray-300 dark:text-gray-600">(empty = all)</span>
          </label>
          <div v-for="(svcs, secName) in servicesBySectionMap" :key="secName" class="mb-3">
            <div class="text-xs text-gray-400 dark:text-gray-600 uppercase tracking-wider mb-1.5">{{ secName }}</div>
            <div class="space-y-1 pl-2">
              <label v-for="svc in svcs" :key="svc.id" class="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" :value="svc.id" v-model="form.service_ids" class="rounded" />
        ...
        <div class="flex gap-2">
          <button @click="createToken" class="bg-gray-800 hover:bg-gray-700 dark:bg-gray-700 dark:hover:bg-gray-600 text-white text-xs px-4 py-1.5 rounded-lg transition-colors">Generate</button>
      ...
      >
        <div class="flex-1 min-w-0">
          ...
          <div class="text-xs text-gray-400 dark:text-gray-600 mt-0.5">
            {{ serviceLabels(token) }} · Last used: {{ formatDate(token.last_used) }}
          </div>
        </div>
        <div class="flex items-center gap-3 ml-4">
          <span class="text-xs px-2 py-0.5 rounded border"
            :class="token.active ? 'text-green-600 dark:text-green-400 border-green-500/30 bg-green-500/10' : 'text-gray-400 border-gray-300 dark:border-gray-700 bg-gray-100 dark:bg-gray-800'">
            {{ token.active ? 'Active' : 'Inactive' }}
          </span>
          <button @click="toggleToken(token)" class="text-xs text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 transition-colors">{{ token.active ? 'Disable' : 'Enable' }}</button>
          <button @click="deleteToken(token.id)" class="text-xs text-red-400 hover:text-red-600 dark:hover:text-red-300 transition-colors">Revoke</button>
        </div>
      </div>
